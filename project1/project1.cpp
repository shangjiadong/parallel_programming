...
int main(int argc, char *argv[])
{
	...
	for (int t = 0; t < NUMTRIES; t++)
	{
		...
		#pragma omp parallel for default(none), shared(fullTileArea),reduction(+:totVolume)
		for (int i = 0; i < NUMNODES*NUMNODES; i++)
		{
			...
			int iv = i / NUMNODES;
			// for the corner
			if ((iu == 0 && iv == 0) || (iu == 0 && iv == NUMNODES) || (iu == NUMNODES && iv == 0) || (iu == NUMNODES && iv == 0))
			{
				volume = Height(iu, iv) * 0.25;
			}
			// for the center
			else if ((iu != 0) && (iu != NUMNODES) && (iv !=0) &&( iv  != NUMNODES))
			{			
				volume = Height(iu, iv);
			}
			// for the edge
			...
			{
				volume = Height(iu, iv) * 0.5;
			}
			totVolume += volume;
		}
		double time1 = omp_get_wtime();
		double megaHeights = (float)NUMNODES *(float)NUMNODES / (time1 - time0)/1000000.;
		sumMegaHeights += megaHeights;
		if (megaHeights > maxMegaHeights)
			maxMegaHeights = megaHeights;

		totVolume *= fullTileArea;
		printf("Volume = %8.2lf \n", totVolume);
	}
