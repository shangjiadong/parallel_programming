...
#define	NUM_WORK_GROUPS		2 * SIZE/LOCAL_SIZE
...
main( int argc, char *argv[ ] )
{
	...
	{
		hArray[i] = hArray[i+Size] = (float) sqrt(  (double)i  );
	}

	size_t dataSizeArray = 2 * Size * sizeof(float);
	size_t dataSizeSums = Size * sizeof(float);
	...
	status = clEnqueueWriteBuffer( cmdQueue, dArray, CL_FALSE, 0, dataSizeArray, hArray, 0, NULL, NULL );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clEnqueueWriteBuffer failed (1)\n" );

	status = clEnqueueWriteBuffer( cmdQueue, dSums, CL_FALSE, 0, dataSizeSums, hSums, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	...
	status = clEnqueueReadBuffer( cmdQueue, dSums, CL_TRUE, 0, dataSizeSums, hSums, 0, NULL, NULL );
	if( status != CL_SUCCESS )
